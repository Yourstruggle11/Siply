...
Every key is verified with a small provider request before it is saved. Ask Siply provides locally selected suggested questions, keeps only the current on-screen conversation, automatically follows new messages unless the user has scrolled up, and sends at most three prior question/answer pairs; chat transcripts are not persisted. If a provider reports that an answer hit its output limit, the partial answer is retained with an explicit **Continue** action—the app never starts an additional paid request without the user's tap. AI context contains bounded daily totals, seven days of hourly aggregates, the daily target, active window, display unit, gentle-goal state, streaks, and other derived statistics—not raw log IDs, exact entry timestamps, or hidden internal reminder-calculation values. The product-aware prompt prohibits invented settings or actions and adds medical or capability limitations only when they are relevant to the question.

Automatic AI features are off by default and run only while History is visible, a connected key is usable, and the device is online. The current-day annotation has a separate three-attempt budget divided into early, middle, and late portions of the active window (0–40%, 40–75%, and 75–100%). At most one automatic request can start in each portion, reopening History cannot spend another opportunity in the same portion, and missed portions are not caught up later. After the first result, a later portion refreshes only after at least 350 ml of accumulated intake change, a target crossing, or a meaningful history/settings change. A failed request consumes only its current portion. The latest valid result remains visible through small same-day changes; it is not carried into another day or across a target change. If all three opportunities are spent without usable text, **Refresh AI insight** lets the user explicitly choose whether to make another request.

History also provides a recap of the previous completed calendar day. It can be dismissed for that day without deleting its data or cached AI text. On Sundays only, History can show a review of the prior completed Sunday-to-Saturday week after at least four tracked days, with its date range visible. These stable historical artifacts use separate one-attempt-per-period budgets, so they do not consume the three current-day History opportunities or regenerate when today's intake changes. With automatic AI enabled, Siply may add one cached AI recap per completed day and one cached AI review per completed week. Late edits to the reviewed period invalidate its matching AI text. Truncated, malformed, or reasoning-like automatic output is discarded and never cached.

Offline, missing-key, invalid-key, timeout, and provider errors never replace History content with an error. A valid same-day cached result remains visible; otherwise Siply silently shows only the deterministic local content. Removing a key preserves cached output. **Settings → Data & privacy → Backup & saved AI → Delete saved AI insights** explicitly clears History annotations, daily recaps, and weekly reviews.
...
|---|---|---|---|
| Yes, for today | No | Either | Local insight plus the saved **AI insight**; no request |
| Yes, for today | Yes | No | Local insight plus the saved **AI insight**; no request |
| Yes, for today | Yes | Yes | Local insight plus the saved **AI insight**; checks the current phase and meaningful-change rules before refreshing |
| Yes, from another day or target | Either | Either | Local insight only; the inapplicable AI copy is hidden |
